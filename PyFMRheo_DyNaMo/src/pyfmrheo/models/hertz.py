...
class HertzModel:
    ...
    def get_correction_coeffs(self, sample_height, indentation):
        correction_params = [
            sample_height, (indentation - self.delta0), self.ind_geom, self.tip_parameter]
    ...
    def build_params(self):
        params = Parameters()
        params.add('delta0', value=self.delta0_init,
                   min=self.delta0_min, max=self.delta0_max)
        ...
        if self.fit_hline_flag:
            params.add('slope', value=self.slope_init,
                       min=self.slope_min, max=self.slope_max)
        return params
    ...
    def model(self, indentation, delta0, E0, f0, slope=None, sample_height=None):
        ...
        if self.correction_model and self.correction_model == 'kontomaris':
            ...
            correction_coeffs = self.get_correction_coeffs(
                sample_height, indentation)
        else:
        ...
        if self.fit_hline_flag:
            ...
            force[non_contact_mask] = (
                indentation[non_contact_mask] - delta0) * slope + f0
        else:
        ...
        force[contact_mask] = (
            coeff * correction_coeffs[contact_mask] * E0 *
            np.power((indentation[contact_mask] - delta0), n) + f0
        )
    ...
    def fit(self, indentation, force, sample_height=None):
        ...
        if self.fit_hline_flag:
            hertzmodel =\
                lambda indentation, delta0, E0, f0, slope: self.model(
                    indentation, delta0, E0, f0, slope, self.sample_height)
        else:
            hertzmodel =\
                lambda indentation, delta0, E0, f0: self.model(
                    indentation, delta0, E0, f0, self.slope, self.sample_height)
        ...
        result_hertz = hertzmodelfit.fit(
            force, params, indentation=indentation, method=self.fit_method)
