...
class TingModel:
    def __init__(self, ind_geom, tip_param, modelFt) -> None:
        ...
        # If radius units is meters, If half angle units is degrees
        self.tip_parameter = tip_param
        self.modelFt = modelFt
        # leastsq is the default, Least-Squares minimization, using Trust Region Reflective method
    ...
    def build_params(self):
        ...
        params.add('betaE', value=self.betaE_init,
                   min=self.betaE_min, max=self.betaE_max)
    ...
    def SolveAnalytical(self, ttc, trc, t1, model_probe, geom_coeff, v0t, v0r, v0, E0, betaE, t0, F0, vdrag):
        ...
        if model_probe == 'paraboloid':
            Cp = 1/geom_coeff
            Ftp = 3/2*v0t**(3/2)*E0*t0**betaE*np.sqrt(np.pi)*np.array(gamma(1-betaE),
                                                                      dtype=float)/(Cp*2*np.array(gamma(5/2-betaE), dtype=float))*ttc**(3/2-betaE)
        ...
        # Conical/Pyramidal geometry
            ...
            Cc = 1/geom_coeff
            if np.abs(v0r-v0t)/v0t < 0.01:
                Ftc = 2*v0**2*E0*t0**betaE/Cc / \
                    (2-3*betaE+betaE**2)*ttc**(2-betaE)
                Frc = -2*v0**2.*E0*t0**betaE/Cc/(2-3*betaE+betaE**2)*((trc-t1)**(1-betaE)*(trc+(1-betaE)*t1) -
                                                                      trc**(1-betaE)*(trc))
            else:
                Ftc = 2*v0t**2*E0*t0**betaE/Cc / \
                    (2-3*betaE+betaE**2)*ttc**(2-betaE)
    ...
    def SolveNumerical(self, delta, time_, geom_coeff, geom_exp, v0t, v0r, E0, betaE, F0, vdrag, smooth_w, idx_tm, idxCt, idxCr):
        ...
        A = smooth(np.r_[numdiff(delta0[idxCt]**geom_exp),
                   numdiff(delta0[idxCr[0]:]**geom_exp)], smooth_w)
        if len(A) < len(delta_Uto_dot[idxCt[0]:]):
            A = np.append(A, A[-1])
        delta_Uto_dot[idxCt[0]:] = A
        delta_dot = np.zeros(len(delta0))
        B = smooth(np.r_[numdiff(delta0[idxCt]),
                   numdiff(delta0[idxCr[0]:])], smooth_w)
    ...
    def model(
        ...
        geom_coeff, geom_exp = get_coeff(
            self.ind_geom, self.tip_parameter, self.poisson_ratio)
        ...
        if modelFt == 'analytical':
            ...
            print(
                f'The modelFt {modelFt} is not supported. Current valid modelFt: analytical, numerical.')
    ...
    def fit(self, time, F, delta, t0, idx_tm=None, smooth_w=None, v0t=None, v0r=None):
        ...
        tingmodel =\
            lambda time, E0, tc, betaE, F0: self.model(
                time, E0, tc, betaE, F0, **fixed_params)
        ...
        result_ting = tingmodelfit.fit(
            F, params, time=time, method=self.fit_method)
        ...
        # Compute metrics
        modelPredictions = self.eval(
            time, F, delta, t0, idx_tm, smooth_w, v0t, v0r)
        ...
        self.chisq = self.get_chisq(
            time, F, delta, t0, idx_tm, smooth_w, v0t, v0r)
        self.redchi = self.get_red_chisq(
            time, F, delta, t0, idx_tm, smooth_w, v0t, v0r)
    ...
    def get_chisq(self, time, F, delta, t0, idx_tm=None, smooth_w=None, v0t=None, v0r=None):
        a = (self.get_residuals(time, F, delta,
             t0, idx_tm, smooth_w, v0t, v0r)**2/F)
